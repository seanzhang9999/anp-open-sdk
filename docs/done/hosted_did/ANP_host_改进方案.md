...
```python
# anp_open_sdk_framework/server/router/router_host.py
# 在现有文件基础上添加HTTP申请接口
...
async def submit_hosted_did_request(request: Request, hosted_request: HostedDIDRequest):
    ...
    try:
        ...
        # 使用队列管理器处理申请
        from anp_open_sdk_framework.server.publisher.hosted_did_queue_manager import HostedDIDQueueManager
...
async def check_hosted_did_result(request: Request, requester_did_id: str):
    ...
    try:
        ...
        from anp_open_sdk_framework.server.publisher.hosted_did_result_manager import HostedDIDResultManager
...
```python
# anp_open_sdk_framework/server/anp_server_hoster.py
...
```python
# anp_open_sdk_framework/server/publisher/hosted_did_queue_manager.py
...
```python
# anp_open_sdk_framework/server/publisher/hosted_did_result_manager.py
...
```python
# anp_open_sdk_framework/server/publisher/hosted_did_processor.py
...
```python
# anp_open_sdk/anp_user.py (添加到ANPUser类)
...
async def request_hosted_did_async(self, target_host: str, target_port: int = 9527) -> Tuple[bool, str, str]:
    ...
    try:
        ...
        # 发送申请请求
        target_url = f"http://{target_host}:{target_port}/publisher/hosted-did/request"
...
```python
# anp_open_sdk_framework/server/publisher/anp_sdk_publisher.py
# 修改现有函数，添加HTTP选项
...
```python
# 导入现有的托管逻辑
from anp_open_sdk_framework.server.anp_server_hoster import (
