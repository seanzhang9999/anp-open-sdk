...
from anp_server.anp_server import ANP_Server, ANPUser
from anp_server_framework.anp_service.agent_api_call import agent_api_call_post, agent_api_call_get
from anp_server_framework.anp_service.agent_message_p2p import agent_msg_post
from anp_server_framework.anp_service.anp_tool import ANPTool
from .step_helper import DemoStepHelper



from demo_anp_sdk.demo_modules.customized_group_member import (
    GroupMemberWithStorage,
    GroupMemberWithStats,
    GroupMemberComplete
)
from demo_anp_sdk.demo_modules.customized_group_runner import (
...
class DemoTaskRunner:
    ...
    async def run_agent_lifecycle_demo(self, agent1,agent2,agent3):
        ...
        try:
            ...
            try:
                ...
                if temp_agent:
                    # 从SDK中注销
                    await self.sdk.unregister_agent(temp_agent.id)
    ...
    async def run_message_demo(self, agent2: ANPUser, agent3: ANPUser, agent1: ANPUser):
        ...
        resp = await agent_msg_post(agent2.id, agent3.id, f"你好，我是{agent2.name}")
        ...
        resp = await agent_msg_post(agent3.id, agent1.id, f"你好，我是{agent3.name}")
    ...
    async def run_group_chat_demo(self, agent1: ANPUser, agent2: ANPUser, agent3: ANPUser):
        ...
        try:
            ...
            logger.debug("-" * 40)
            group_log_files = [
                UnifiedConfig.resolve_path("demo_anp_sdk/data_tmp_result/group_logs/sample_group_messages.json"),
                UnifiedConfig.resolve_path("demo_anp_sdk/data_tmp_result/group_logs/moderated_group_messages.json")
            ]
            ...
            for agent, agent_prefix, agent_type in storage_agents:
                if agent_type in ["GroupMemberWithStorage", "GroupMemberComplete"]:
                    message_file = UnifiedConfig.resolve_path(f"demo_anp_sdk/data_tmp_result/member_messages/{agent_prefix}_group_messages.json")
    ...
    async def clean_demo_data(self):
        ...
        try:
            # 获取demo_data目录路径
            demo_data_path = UnifiedConfig.resolve_path("demo_anp_sdk/data_tmp_result")
